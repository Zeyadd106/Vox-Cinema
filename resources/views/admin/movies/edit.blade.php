        ...
        <form action="{{ route('admin.movies.update', $movie->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="duration" id="duration" value="{{ old('duration', $movie->duration) }}" required min="1" max="999"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                </div>
                ...
                <div>
                    ...
                    <select name="rating" id="rating" required
                        ...
                        @foreach(['G', 'PG', 'PG-13', 'R', 'NC-17'] as $rating)
                            <option value="{{ $rating }}" {{ old('rating', $movie->rating) == $rating ? 'selected' : '' }}>{{ $rating }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        <option value="current" {{ old('status', $movie->status) == 'current' ? 'selected' : '' }}>Currently Showing</option>
                ...
                <div>
                    ...
                    <input type="url" name="trailer_url" id="trailer_url" value="{{ old('trailer_url', $movie->trailer_url) }}" required
                ...
                <div>
                    ...
                    <input type="file" name="poster" id="poster" accept="image/jpeg,image/png,image/jpg,image/gif"
                    ...
                    @if($movie->poster_path)
                        <div class="mt-2">
                            <img src="{{ Storage::url($movie->poster_path) }}" alt="Current poster" class="h-32 w-auto object-cover rounded">
                            <p class="mt-1 text-sm text-gray-500">Current poster</p>
                        </div>
                    @endif
                </div>
            </div>
