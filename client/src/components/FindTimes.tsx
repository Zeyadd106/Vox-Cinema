...
export default function FindTimes() {
  ...
  return (
    <div className="mx-auto max-w-6xl rounded-xl border border-[#333] bg-[#141414] p-6">
      <h2 className="mb-4 text-center text-xl font-bold uppercase tracking-widest text-vox">{t.find.title}</h2>
      <form onSubmit={search} className="grid gap-3 md:grid-cols-[1fr_1fr_auto]">
        <label className="text-sm">
          <span className="mb-1 block text-xs text-[#999]">{t.find.movie}</span>
          <select value={movie} onChange={(e) => setMovie(e.target.value)} className={`${select} w-full`}>
            <option value="any">{t.find.anyMovie}</option>
            {movies.map((m) => <option key={m.id} value={m.id}>{m.title}</option>)}
          </select>
        </label>
        <label className="text-sm">
          <span className="mb-1 block text-xs text-[#999]">{t.find.date}</span>
      ...
      {results !== null && (
        <div className="mt-6 space-y-5">
          {results.length === 0 && <p className="text-center text-sm text-[#888]">{t.find.noMatch}</p>}
