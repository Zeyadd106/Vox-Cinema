...
export default function AdminShowtimes() {
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold">Showtimes</h1>
      ...
      <form onSubmit={create} className="mb-8 flex flex-wrap items-end gap-3 rounded-lg border border-[#333] bg-[#1a1a1a] p-5">
        <div><label className="mb-1 block text-xs text-[#aaa]">Movie</label>
          <select required value={form.movie_id} onChange={(e) => setForm({ ...form, movie_id: e.target.value })} className={input}>
            <option value="">Select...</option>
            {movies.map((m) => <option key={m.id} value={m.id}>{m.title}</option>)}
          </select></div>
        <div><label className="mb-1 block text-xs text-[#aaa]">Hall</label>
        ...
        <div><label className="mb-1 block text-xs text-[#aaa]">Date</label>
          <input type="date" required value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className={input} /></div>
        <div><label className="mb-1 block text-xs text-[#aaa]">Time</label>
          <input type="time" required value={form.time} onChange={(e) => setForm({ ...form, time: e.target.value })} className={input} /></div>
        <button className="rounded-md bg-vox px-6 py-2 font-semibold hover:bg-vox-dark">Add</button>
      </form>
      <div className="overflow-x-auto rounded-lg border border-[#333]">
        <table className="w-full text-left text-sm">
          <thead className="bg-[#222] text-[#aaa]">
            <tr><th className="px-4 py-2.5">Movie</th><th className="px-4 py-2.5">Cinema</th><th className="px-4 py-2.5">Hall</th><th className="px-4 py-2.5">Date</th><th className="px-4 py-2.5">Time</th><th className="px-4 py-2.5">Actions</th></tr>
          </thead>
          <tbody>
            {showtimes.map((s) => (
              <tr key={s.id} className="border-t border-[#2a2a2a]">
                ...
                <td className="px-4 py-2.5"><button onClick={() => remove(s.id)} className="text-red-400 hover:underline">Delete</button></td>
              </tr>
            ))}
          </tbody>
        </table>
